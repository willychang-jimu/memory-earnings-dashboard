...
`highlightMatches()` 走 TreeWalker 逐個**文字節點**包 `<mark class="hit">`，
**不要改成用 innerHTML 拼字串**：一來會毀掉既有的事件處理（勾選框、展開收合），
二來內容含外部來源資料，拼 HTML 等於開一個注入破口。

**`clearHighlights()` 必須在每次搜尋前呼叫**，不能只靠 `renderAll()` ——
寫死在 `index.html` 的卡片說明文字不會被重畫，舊標記會殘留，
症狀是分頁徽章的加總跟「找到 N 筆」對不上（實際發生過）。

搜尋輸入走 `applySearch()` 而不是 `renderAll()`：只重標記比較快，
也不會讓使用者的收折狀態跳掉。但 `renderAll()` 結尾要呼叫 `applySearch()`，
否則切換公司篩選後標記會消失。
